...
fig_mrr_dg <- function(psy_in = 3, sukup_in, dodge_in = 0, limits_in = c(0,10)){
  dat$fig_mrr_dg %>% 
...
fig_mrr_dg_plotly <- function(psy_in =3, limits = c(0,6)){
  men <- ggplotly(
  fig_mrr_dg(psy_in = psy_in, sukup_in = c('Men'), limits_in = limits),
  tooltip = c('text')
  ) %>% layout(
...
women <- ggplotly(
  fig_mrr_dg(psy_in = psy_in, sukup_in = c('Women'), limits_in = limits)+theme(axis.text.y = element_blank()),
  tooltip = c('text')
  ) %>% layout(
    legend = opt_fig$dg$legend,
    yaxis = list(fixedrange = TRUE),
...
fig_mrr_dg_plotly(psy_in = 2, limits = c(0,9))

```
<div style="color:darkred;text-align:center">Note X axis values</div>
